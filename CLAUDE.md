...
`SuperCamera.synthesize_ir(mode, ambient_temp)` returns a `float32 (H,W)` array in **`[0,1]`** built purely from per-buffer render data — **no calibrated radiometry, no noise**. `mode` is a **spectral-band name** (case-insensitive); each band has its own physically-motivated `_synth_*` model. The final output is normalized in `_compute_ir` by a **robust white point** — the 99th percentile of the valid (non-background) pixels, not the true max (`out / out.max()`) — so a few outlier-bright pixels (specular glints, `T⁴` hot spots, near-camera `1/d²` spikes) saturate to white instead of crushing the whole frame dark; black stays pinned at 0 (no floor stretch). Always directly displayable. Required buffers are auto-attached on first call.
...
- **`MWIR` (3000–5000 nm):** `ir = ε' · T⁴ · τ`. `ε = _emissivity()` (rough/diffuse → high, smooth/specular → low); `ε' = ε · (1 − _LWIR_GRAZING_EMISSIVITY_WEIGHT·(1−N·V))`, the same grazing-angle darkening term as LWIR (see below). `T = ambient + _MWIR_EMISSIVE_GAIN·emissive_heat + _MWIR_MOTION_GAIN·motion_heat`. The `T⁴` power **biases hard toward hot objects**; cool ambient stays dim.
- **`LWIR` (8000–14000 nm):** `ir = ε' · T · geom · τ`. Near-**linear** in temperature so the whole scene glows; `T = ambient + _LWIR_EMISSIVE_GAIN·emissive_heat + _LWIR_MOTION_GAIN·motion_heat`; `geom = (1−_LWIR_GEOMETRY_WEIGHT) + _LWIR_GEOMETRY_WEIGHT·N·V` (weak geometry). **No** inverse-square falloff (emitted, not illuminated). `ε' = ε · (1 − _LWIR_GRAZING_EMISSIVITY_WEIGHT·(1−N·V))` — a **grazing-angle emissivity darkening** (stylized Fresnel-like effect, default weight `0.3`) applied only inside `_synth_lwir`, on top of the shared `_emissivity()`; it does not affect `_emissivity()` itself or MWIR (which stays isotropic). Mimics the edge-darkening silhouette look of real thermal footage — a deliberate visual-style choice, not a radiometric one.
...
- `synthesize_ir` output is already normalized to `[0,1]` (by the 99th-percentile robust white point, not `out.max()` — see "Synthetic image generation" above) and **has no noise**. The GUI (`_on_capture`) clips it (`np.clip(ir, 0, 1)`) and runs it through `SuperCamera.colorize(disp, "ironbow")` → uint8 `(H,W,3)` thermal RGB, which it both previews and saves. The colormap is a **fixed LUT, not AGC** — it maps each `[0,1]` value to a fixed colour, so it does NOT reintroduce the percentile-stretch static. No `to_display` stretch in the capture path.
